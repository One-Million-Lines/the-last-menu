...
function B(c,m){C=Object.assign({},D,c);S=[];var mt=m||document.body;N=E("div","lm-root");N.setAttribute("data-theme",C.theme);N.setAttribute("data-pos",C.position);if(C.showIcons===false)N.classList.add("lm-no-icons");var o=E("div","lm-overlay");o.addEventListener("click",function(){O(false)});N.appendChild(o);var p=E("nav","lm-panel");p.setAttribute("aria-label",C.title);if(C.scrollable)p.classList.add("lm-scrollable");N.appendChild(p);W=C.items;T=C.title;P(C.items,C.title,false);var f=E("button","lm-fab");f.setAttribute("aria-label","Toggle menu");f.innerHTML='<svg class="lm-icon-menu" viewBox="0 0 24 24"><path d="M4 7h16M4 12h16M4 17h16"></path></svg><svg class="lm-icon-x" viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18"></path></svg>';f.addEventListener("click",function(){O(!Z)});N.appendChild(f);mt.appendChild(N)}
